...
placeOrder(orderDto: any): Observable<any> {
  ...
  // 🔥 Vérifie ce que tu envoies
  console.log("DATA SENT:", orderDto);
...
getWishlistByUserId(): Observable<any> {
  ...
  return this.http.get(`${BASIC_URL}/api/customer/wishlist/${userId}`, {
    ...
    console.log("Token envoyé au back:", token); // 👈 vérifie que le token existe
    return new HttpHeaders().set(
      'Authorization', `Bearer ${token}`  // note l'espace ici
    );
  }
}
